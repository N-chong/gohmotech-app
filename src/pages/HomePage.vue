    ...
    <ion-content>
      ...
      <main class="page-wrap home-dashboard">
        ...
        <template v-else-if="data">
          ...
          <section class="dashboard-section"><div class="rich-section-heading"><div><p class="eyebrow">CONTEXTUAL CONTROL</p><h2>Goat house operations</h2></div><router-link to="/app/automation">All controls</router-link></div><div class="operation-grid"><router-link class="operation-tile door" to="/app/automation"><span><ion-icon :icon="enterOutline" /></span><div><small>Goat house door</small><strong>{{ titleCase(data.automation.door?.state || 'unknown') }}</strong></div><ion-icon :icon="chevronForwardOutline" /></router-link><router-link class="operation-tile light" to="/app/automation"><span><ion-icon :icon="bulbOutline" /></span><div><small>Lighting</small><strong>{{ titleCase(data.automation.light?.state || 'unknown') }}</strong></div><ion-icon :icon="chevronForwardOutline" /></router-link><router-link class="operation-tile camera" to="/app/live"><span><ion-icon :icon="cameraOutline" /></span><div><small>Live monitoring</small><strong>{{ data.system.cameras_online }} online</strong></div><ion-icon :icon="chevronForwardOutline" /></router-link><router-link class="operation-tile security" to="/app/security"><span><ion-icon :icon="shieldCheckmarkOutline" /></span><div><small>Security</small><strong>{{ criticalAlerts ? `${criticalAlerts} priority alerts` : 'All clear' }}</strong></div><ion-icon :icon="chevronForwardOutline" /></router-link></div></section>
